...
async function loadProducts(page = 1) {
  try {
    ...
    if (state.selectedCategories.length > 0) {
      // Parallel fetch for selected categories
      const requests = state.selectedCategories.map(cat =>
        fetchProducts({ limit: 100, skip: 0, category: cat })
      );
...
function createCard(product) {
  ...
  return `
    <div class="${isList ? 'col-12' : 'col-12 col-sm-6 col-md-4 col-lg-3'} mb-3 d-flex">
